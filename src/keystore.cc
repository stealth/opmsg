...
persona *keystore::add_persona(const string &name, const string &c_pub_pem, const string &priv_pem, const string &dhparams_pem)
{
	...
	if (pub_pem.size() > 0) {
		unique_ptr<char, decltype(&free)> sdup{
			strdup(pub_pem.c_str()),
			free
		};
		unique_ptr<BIO, decltype(&BIO_free)> bio{
	...
	if (priv_pem.size() > 0) {
		unique_ptr<char, decltype(&free)> sdup{
			strdup(priv_pem.c_str()),
			free
		};
		unique_ptr<BIO, decltype(&BIO_free)> bio{
			BIO_new_mem_buf(sdup.get(), priv_pem.size()),
			BIO_free
		};
		if (!bio.get())
			return build_error("add_persona: OOM", nullptr);
...
int persona::make_pq1(const string &type, const string &secret, const string &salt)
{
	...
	unique_ptr<FILE, decltype(&ffclose)> f{
		fopen(pqc.c_str(), "w"),
		ffclose
	};
	if (!f.get())
